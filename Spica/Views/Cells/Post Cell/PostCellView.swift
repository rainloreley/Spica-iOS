...
private extension PostCellView {
	...
	func layoutPostImageView(height: Int? = nil) {
		postImageView.snp.removeConstraints()
		postImageView.snp.makeConstraints { (make) in
			make.top.equalTo(postContentTextView.snp.bottom).offset(12)
			make.leading.equalTo(contentView.snp.leading).offset(16)
			make.trailing.equalTo(contentView.snp.trailing).offset(-16)
			make.height.equalTo(postImageView.image != nil ? Int(postImageView.image!.size.height / 3) : 20)
		}
	}
	
	func layoutPostLinkBackgroundView(hidden: Bool = false, remake: Bool = false) {
