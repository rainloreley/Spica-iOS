...
class FollowersFollowingViewController: UIViewController {
	...
	func makeDataSource() -> DataSource {
		let source = DataSource(tableView: tableView) { [self] (tableView, indexPath, user) -> UITableViewCell? in
			let cell = tableView.dequeueReusableCell(withIdentifier: "cell", for: indexPath) as! UITableViewCell
			
			var content = UIHostingController(rootView: FollowerCell(follower: user)).view
